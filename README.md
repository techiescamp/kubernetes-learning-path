...
🚀  CKA, CKAD, CKS, or KCNA exam aspirants can **save $80** (25%) today using code **DCUBE20** at https://kube.promo/devops. It is a limited-time offer from the Linux Foundation.
...
Creating Kubernetes(K8) cluster is equally important as creating logs and choosing right kind of monitoring tools. As a devops engineer we should think about that from initially. I've been mentioning couple of tools here to see if we can use it synchronously.

- **Cloudwatch Logs for K8:** [Kubernetes Cloudwatch Explained](https://saturncloud.io/blog/how-to-send-kubernetes-logs-to-aws-cloudwatch/)

Kubernetes is a container orchestration platform that automates the deployment, scaling, and management of containerized applications. Kubernetes generates a lot of logs, which can be challenging to manage. Sending your Kubernetes logs to AWS CloudWatch allows you to centralize your log management and take advantage of the powerful features and integrations provided by AWS CloudWatch.

The CloudWatch Agent is a lightweight agent that runs on your servers and collects metrics and logs. It supports various log types, including JSON, syslog, and Apache. The CloudWatch Agent has a Kubernetes log collection feature that allows you to collect logs from your Kubernetes containers.

To send Kubernetes logs to AWS CloudWatch using the CloudWatch Agent, follow these steps:

    Install the CloudWatch Agent on your Kubernetes cluster.

    Configure the agent to collect logs from your Kubernetes containers.

Here’s an example CloudWatch Agent configuration file:
                ...
                "collect_list": [
                    {
                        ...
                        "log_stream_name": "{hostname}-kubernetes",
...
```
This configuration file tells the CloudWatch Agent to collect logs from the /var/log/containers/*.log path and send them to the my-log-group log group. The log_stream_name parameter includes the hostname of the container and the word “kubernetes” to uniquely identify the log stream. The timestamp_format parameter specifies the format of the timestamp in the log files.
